...
public class PatricipatesRun {
    public static void main(String[] args) {
        ...
        try {
            ...
            user.addMessenger(new UserMessenger("tg", "@tg"));
            user.addMessenger(new UserMessenger("wu", "@wu"));
            user.setRole(role);
            create(user, sf);
            ...
            Item foundItem = session.createQuery("FROM Item WHERE id = :fId", Item.class)
                    .setParameter("fId", item.getId())
                    .getSingleResult();

            for (User u : foundItem.getParticipates()) {
                u.getRole().getName();
                u.getMessengers().size();
            }

            session.getTransaction().commit();
            session.close();

            foundItem.getParticipates().forEach(System.out::println);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
